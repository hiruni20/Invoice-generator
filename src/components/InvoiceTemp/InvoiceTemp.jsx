...
function InvoiceTemp() {
      ...
      <div className='upper'>
        <div className='upper-text'>
          ...
          <div className='upper-p'>
            <p>Welcome to the original Invoice Generator, trusted by millions of people. Invoice Generator lets you instantly make invoices 
                with our attractive invoice template straight from your web browser. The invoices you make can be sent and paid online or downloaded as a PDF.</p>
            <p>Did we also mention that Invoice Generator lets you generate an unlimited number of invoices?</p>
          </div>
          <div classname='upper-btn'>
                <button style={{height:'3em', fontSize:'18px'}}>OK, Got it</button>
                <br/><br/>
            </div>
        </div>
      </div>
        ...
        <form className="invoice-editor">
          <div className="header">
            <div className='header-inputs' style={{width:'100%'}} >
              ...
              <div className='form-column' style={{ height: '100px', width: '100px', paddingRight: '5px' }}>
                <input type="file" id="logo" accept="image/*" className='logo' onChange={handleLogoChange} />
                {logoUrl ? (
                    <>
                      <img src={logoUrl} alt="Uploaded Logo" style={{ maxWidth: '100%', maxHeight: '100%', width: 'auto', height: 'auto' }} />
                    <button className="remove-logo-btn" onClick={handleRemoveLogo}>X</button>
                    </>
                ) : (
                    <label htmlFor="logo" className="logo-label" style={{ height: '100px', width: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        + Add your logo
                    </label>
                )}
            </div>
          ...
          <div className="details" style={{marginTop: '-50px', position: 'relative', zIndex: '1'}}>
      
            <div className='form-row' ><input type="text" placeholder="Who is this invoice from? required" required  style={{width:'400px'}} /></div>
          <div className='form-row' > 
            <div className='form-row'>              
                <div className='form-column'>
                    <input type="text" placeholder="Bill To" required  className='col-in' style={{width:'260px', height:'46px'}}/>
                  </div>
                  <div className='form-column'>
                    <input type="text" placeholder="Ship To " className='col-in' style={{width:'260px', height:'46px'}}/>
                  </div>                
              </div>

            <div className='form-column' style={{paddingRight:'40px',position:'', }}>
              <div className='form-row'>
              ...
              <div className='form-row'>
                <input type="text" placeholder="Payment Terms" className='col-in' style={{width:'50px', height:'36px'}} />              
                <input type="text" placeholder=""  style={{width:'200px', height:'36px'}} /> 
              </div>
              <div className='form-row'>
                <input type="text" placeholder="Due Date" className='col-in' style={{width:'50px', height:'36px'}} />              
                <input type="text" placeholder=""  style={{width:'200px', height:'36px'}} /> 
              </div>
              <div className='form-row'>
                <input type="text" placeholder="PO Number" className='col-in' style={{width:'50px', height:'36px'}} />              
                <input type="text" placeholder=""  style={{width:'200px', height:'36px'}} /> 
              </div>
              </div>
            ...
            <div className='form-row' style={{marginTop: '-130px', position: 'relative', zIndex: '3'}}>
              < div className='form-column' style={{marginRight:'-330px'}}>
                  <input type="text" placeholder="Who is this invoice from? required" required  style={{width:'256px'}} />
              </div>
              < div className='form-column'>
                  <input type="text" placeholder="Who is this invoice from? required" required  style={{width:'260px'}} />
              </div>
  ...
  {items.map((item, index) => (
    <div className="item-row" key={index}>
      ...
      <button type="button" onClick={() => removeItem(index)} className='remove-btn'>x</button>
    </div>
  ))}
  
  <button type="button" onClick={addItem} style={{ backgroundColor: '#fff', color: '#198754', border: '1px solid #198754' }}>+ Line Item</button>
</div>

          <div className="totals">
            <div className='container-right'>
            <div className='form-column'>
                    <input type="text" placeholder="Notes" required  className='col-in' style={{width:'260px', height:'46px'}}/>
                  </div>
                  < div className='form-column'>
                  <input type="text" placeholder="Notes - any relevant information not already covered" required  style={{width:'500px',height:'60px', marginTop:'10px',marginBottom:'10px'}} />
              </div>
              <div className='form-column'>
                    <input type="text" placeholder="Terms" required  className='col-in' style={{width:'260px', height:'46px'}}/>
                  </div>
                  < div className='form-column'>
                  <input type="text" placeholder="Terms and conditions - Late fees, payment metods,delivery shedule" required  style={{width:'500px',height:'60px', marginTop:'10px',marginBottom:'10px'}} />
              </div>
            ...
            <div className="total-row">
              ...
              {showAddDiscountButton && (
                <button type="button" onClick={addDiscount} style={{ backgroundColor: '#fff', color: '#198754', border: 'none' }}>+ Discount</button>
              )}
              </div>
              
              <div className="form-row">
              {showAddShipping && (
                <button type="button" onClick={addShipping} style={{ backgroundColor: '#fff', color: '#198754', border: 'none' }}>+ Shiping</button>
              )}
              </div>
            </div>
            {discounts.map((discount, index) => (
              <div className="total-row" key={index}>
                ...
                <button type="button" onClick={() => removeDiscount(index)} className='remove-btn'>x</button>
            ...
            {shipping.map((shipping, index) => (
              <div className="total-row" key={index}>
                ...
                <button type="button" onClick={() => removeShipping(index)} className='remove-btn'>x</button>
      ...
      <div className='submit-container'>
          ...
          <div className='selector'>
                <label style={{marginTop:'5px', color:"#b8b7b7"}}>Currency</label>
                <select style={{width:'160px',height:'35px'}}value={currency} onChange={(e) => setCurrency(e.target.value)}className='optn'>
